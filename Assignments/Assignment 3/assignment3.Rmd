...
```{r}
ldaData <- lda(Type~., Glass[2:11])
ldaData
```

1.c.ii.  I need to watch the videos...

But I think this means you can predict the type by taking the data and doing the following linear transformation on it 

$$
311.7*RI+2.4*Na + 0.7*Mg + 3.3*Al + 2.5*Si + 1.6*K +1* Ca +2.3*Ba - .5*Fe
$$

1.c.iii.
```{r}
values <- predict(ldaData, Glass[2:11])
values$x[,1]
