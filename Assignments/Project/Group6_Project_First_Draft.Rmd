...
dfBy10min <-df %>%
  group_by(group10min = cut(dt, "15 min") #Divide the datetime column into 15-minute intervals,Staff) %>% #Include the staff variable to analyze the impact of staff on sales ]
  summarize (names = list(unique(str_trim(Name))), #Collect unique item names sold during each interval
    totalSales = sum(Final.price..USD.), #Calculate total sales for each 15-minute interval
    month, #Include the month as a feature for seasonal analysis
    weekdays #Include the day of the week to understand weekly demand patterns
  ) %>%
  mutatehour = factor(hour(group10min)), #Extract the hour of each 15-minute interval as a factor
