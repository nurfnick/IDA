...
Our business partner would love to have a demand schedule based on the time of day and week.  We will build a model for that.  We utilized time of day in 15 minute, month, and day with location to predict the total sales.  We believe this will be a boon to our corporate sponsor. 
```{r}
#I want to group the data by 15 minute chunks so that I can accurately predict how to allocate workers. 
dfBy10min <-df %>%
  group_by(group10min = cut(dt, "15 min"),Staff) %>% #Include the staff variable to analyze the impact of staff on sales ]
  ...
  mutate(hour = factor(hour(group10min)), #Extract the hour of each 15-minute interval as a factor
    min = minute(group10min) #Extract the minute of each 15-minute interval as a numeric value
  )%>%
  ungroup()
```
...
linear <- lm(
  ...
  data = dfBy10min) #Use the grouped and aggregated dataset for modeling
...
# Modeling Summary
 ...
 -**3. Association Rule Mining**:
    We uncovered relationships between purchased items to help with selling strategies. 
 ...
 -**4. Regression Modeling**:
    ...
    We found that interaction effects revealed great predictive opportunities for predicting sales.
