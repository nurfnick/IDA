...
dfBy10min <-df %>%
  group_by(group10min = cut(dt, "15 min"),Staff) %>%
  summarize(names = list(unique(str_trim(Name))),totalSales = sum(Final.price..USD.), month, weekdays)%>%
  mutate(hour = factor(hour(group10min)), min = minute(group10min))%>%
  ungroup()
  #summarize(totalSales = sum(Final.price..USD.)) %>%

lm(totalSales ~ month+ weekdays + Staff + hour + min +hour:min, data =dfBy10min)
```
