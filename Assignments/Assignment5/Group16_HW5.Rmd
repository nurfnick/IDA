...
summary(OLM)
```
There are lots of variables here.  Let's see if we can reduce the number of variables.  The following code starts with the complete model from above and addes or removes features based on $AIC$, attempting to minimize this value.  We have hidden the results because they were very long.
```{r, results = 'hide'}
...
ggplot(housingData, aes(x = YearBuilt, y = logSalePrice, color = Neighborhood)) +
...
```{r, include = FALSE}
#OLMwI <- lm(log(SalePrice)~.^2, data = housingData)
#summary(OLMwI)
#fit1 <- stepAIC(OLMwI, direction = "both")
```
