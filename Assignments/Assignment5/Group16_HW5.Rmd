...
fitOLM <- lm(log(SalePrice) ~ ., data= housingData )
...
```{r, results = 'hide'}
OLMSim <- stepAIC(OLM, direction = "both")
```
...
```{r}
fitOLMSim <- lm(OLMSim$call$formula,
             data=housingData)
AIC(fit)
BIC(fit)
```
...
|-----------|---------------------|-----------------|-------:|-------:|
|OLS |lm|N/A|",fitOLM$results$RMSE,fitOLM$results$Rsquared,sep = "|")
cat(tabl)
```
```{r}
